        ...
        return {
            drop: (props, monitor, component) => {
                ...
                if (type === "event") {
                    ...
                    initialEndTime =
                        resourceEvents.headerItems[initialLeftIndex].end;
                ...
                if (cellUnit !== CellUnits.Hour)
                    endTime = localeMoment(
                        resourceEvents.headerItems[leftIndex].start
                    )
                        .hour(23)
                        .minute(59)
                        .second(59)
            ...
            hover: (props, monitor, component) => {
                ...
                const { cellUnit, config, viewType, localeMoment } =
                    schedulerData;
                ...
                if (isEvent) {
                    ...
                    if (config.relativeMove) {
                        newStart = localeMoment(event.start)
                            .add(
                                localeMoment(newStart).diff(
                                    localeMoment(initialStart)
                                ),
                                "ms"
                            )
                            .format(DATETIME_FORMAT);
                    ...
                    newEnd = localeMoment(newStart)
                        .add(
                            localeMoment(event.end).diff(
                                localeMoment(event.start)
                            ),
                            "ms"
                        )
                        .format(DATETIME_FORMAT);
