...
class EventItem extends Component {
    ...
    doStartDrag = (ev) => {
        ...
        const { left, width, leftIndex, rightIndex, schedulerData } =
            this.props;
        ...
        if (newWidth < minWidth) {
            newWidth = minWidth;
            newLeft =
                (rightIndex - 1) * cellWidth + (rightIndex - 1 > 0 ? 2 : 3);
    ...
    stopStartDrag = (ev) => {
        ...
        if (
            ...
            if (count > 0) {
                ...
                while (true) {
                    i++;
                    let tempStart = localeMoment(eventItem.start).add(
                        i,
                        "days"
                    );
    ...
    stopEndDrag = (ev) => {
        ...
        const { headers, cellUnit, events, config, localeMoment } =
            schedulerData;
        let cellWidth = schedulerData.getContentCellWidth();
        let offset = leftIndex > 0 ? 5 : 6;
        let minWidth = cellWidth - offset;
    ...
    startResizable = (props) => {
        ...
        return (
            ...
            isInPopover === false &&
            (eventItem.resizable == undefined ||
                eventItem.resizable !== false) &&
